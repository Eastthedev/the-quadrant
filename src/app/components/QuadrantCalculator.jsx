...
function calcQuadrants(entry, sl, tp, direction = 'long') {
...
export default function QuadrantCalculator() {
  ...
  const quadrants = calcQuadrants(entry, sl, tp, direction)
  ...
  return (
    <div style={{ color: TEXT, padding: '40px 0' }}>
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 20px' }}>
        ...
        {/* Direction toggle */}
        <div style={{ display: 'flex', gap: 1, marginBottom: 20, background: BORDER, borderRadius: 4, padding: 1, width: 'fit-content' }}>
          {['long', 'short'].map(d => (
              ...
              <div style={{ borderTop: `1px solid ${BORDER}`, paddingTop: 16 }}>
                ...
                {accountMode === 'recovery' && (
                  <div style={{
                    ...
                    <span>⚠️</span>
                    <span>Recovery Mode Active: Risk is halved and locked to 0.5% per quadrant account.</span>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 12 }}>
                  <div style={{
                    ...
                    risk per quadrant account: ${fmt(riskAmount, 2)}
                  </div>
                </div>
              </div>
            </div>
        ...
        {quadrants && (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 4, marginBottom: 16, overflow: 'hidden' }}>
              {[
                { label: 'SL range', val: fmt(quadrants[0].risk * 4, dec) },
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: mono, fontSize: 12 }}>
                  <thead>
                    <tr style={{ background: SURFACE2, borderBottom: `1px solid ${BORDER}` }}>
                      <th style={{ padding: '10px 16px', fontSize: 10, color: MUTED, textTransform: 'uppercase', letterSpacing: '0.1em', textAlign: 'left' }}>Quadrant</th>
                  ...
                  <tbody>
                    {quadrants.map((q, idx) => {
                      ...
                      return (
                        <tr key={idx} style={{ borderBottom: `1px solid ${BORDER}` }}>
                          <td style={{ padding: '10px 16px', textAlign: 'left', fontWeight: 600, color: accents[idx] }}>{q.label}</td>
                    ...
                    <tr style={{ background: 'rgba(255, 255, 255, 0.02)', fontWeight: 600 }}>
                      <td style={{ padding: '12px 16px', textAlign: 'left', color: GOLD }}>Total</td>
                      <td style={{ padding: '12px 16px', textAlign: 'right', color: MUTED }}>—</td>
                      <td style={{ padding: '12px 16px', textAlign: 'right', color: MUTED }}>—</td>
                      <td style={{ padding: '12px 16px', textAlign: 'right', color: DANGER }}>-${fmt(riskAmount * 4, 2)}</td>
            ...
            <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 4, overflow: 'hidden', marginBottom: 14 }}>
              <div style={{ padding: '10px 16px', borderBottom: `1px solid ${BORDER}`, display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ fontFamily: mono, fontSize: 10, color: MUTED, textTransform: 'uppercase', letterSpacing: '0.1em' }}>price ladder</span>
              ...
              {quadrants.map((q, i) => {
                ...
                return (
                  <div key={i} style={{ display: 'grid', gridTemplateColumns: '40px 1fr 110px 110px 60px', gap: 8, padding: '10px 16px', borderBottom: `1px solid ${BORDER}`, alignItems: 'center' }}>
                    <span style={{ fontFamily: mono, fontSize: 10, color: accents[i] }}>{q.label}</span>
                    <div style={{ height: 2, background: BORDER, borderRadius: 1 }}>
                      <div style={{ height: 2, background: accents[i], width: `${100 - i * 20}%`, borderRadius: 1 }} />
                    </div>
                    ...
                    <span style={{ fontFamily: mono, fontSize: 12, color: accents[i], textAlign: 'right', fontWeight: 600 }}>1:{q.rr.toFixed(0)}</span>
                  </div>
                )
              })}
              {/* SL floor */}
            ...
            <div style={{ padding: '12px 16px', background: GOLD_DIM, border: `1px solid ${GOLD_BORDER}`, borderRadius: 3 }}>
              ...
              <span style={{ fontFamily: mono, fontSize: 10, color: SECONDARY }}>
                {showLotDetails ? (
                  <>
                    all 4 SL hit = −4R (-${fmt(riskAmount * 4, 2)}). One Q1 win = +{quadrants[0].rr.toFixed(0)}R (+${fmt(riskAmount * quadrants[0].rr, 2)}). Break-even win rate on Q1: {(100 / (quadrants[0].rr + 1)).toFixed(1)}%
                  </>
                ) : (
                  <>
                    all 4 SL hit = −4R. One Q1 win = +{quadrants[0].rr.toFixed(0)}R. Break-even win rate on Q1: {(100 / (quadrants[0].rr + 1)).toFixed(1)}%
                  </>
                )}
              </span>
            </div>
        ...
        {!hasValues && (
          <div style={{ textAlign: 'center', padding: '48px 0' }}>
            <div style={{ fontFamily: mono, fontSize: 11, color: MUTED, letterSpacing: '0.1em' }}>
              enter a signal above to split into quadrants
            </div>
          </div>
        )}
      </div>
